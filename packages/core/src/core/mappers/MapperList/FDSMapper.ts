...
export default class FDSMapper extends Mapper {
    ...
    private readFDSRegister(addr: number): number {
        return this._readFDSRegisterImpl(addr)
    }

    private _readFDSRegisterImpl(addr: number): number {
        switch (addr) {
            case 0x4030:
            ...
            case 0x4031:
                ...
                // 只有在磁盘读取模式下才调度下一次DiskSeek IRQ
                // 避免在游戏运行阶段不必要地触发IRQ检查
                if (this.diskReadMode && this.diskMotorOn) {
                    this.diskSeekIrq = 150
                }
