...
class CPU {
    ...
    reset() {
        const instance = new CPU(this.nes)
    ...
    emulate() {
        ...
        switch (opinf & 0xff) {
            ...
            case 3: {
                ...
                if (this.F_CARRY === 0) {
                    cycleCount += (opaddr & 0xff00) === (addr & 0xff00) ? 1 : 2
                    this.REG_PC = addr
                }
                break
            }
    ...
    setStatus(status: number) {
        this.F_CARRY = (status & 0x01) >> 0
        this.F_DECIMAL = (status & 0x02) >> 1
        this.F_INTERRUPT = (status & 0x04) >> 2
        this.F_OVERFLOW = (status & 0x08) >> 3
        this.F_SIGN = (status & 0x10) >> 4
        this.F_ZERO = (status & 0x20) >> 5
    }
